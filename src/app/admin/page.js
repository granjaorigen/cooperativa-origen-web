...
export default function AdminPage() {
  ...
  const [addHoursModal, setAddHoursModal] = useState(null);
  ...
  if (!session) return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#121212", color: "#fff", padding: 20 }}><div style={{ textAlign: "center" }}><h2>Acceso Administrador</h2><p style={{ color: "#888", margin: "12px 0 20px" }}>Debes iniciar sesión primero.</p><Link href="/catalogo" style={{ background: "var(--gold)", color: "#1a1a1a", padding: "12px 24px", borderRadius: 10, fontWeight: 600, display: "inline-block" }}>Ir al Login</Link></div></div>;
  if (!member?.is_admin) return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#121212", color: "#fff", padding: 20 }}><div style={{ textAlign: "center" }}><div style={{ fontSize: 48, marginBottom: 16 }}>🔒</div><h2>Sin permisos</h2><Link href="/catalogo" style={{ background: "var(--green-700)", color: "#fff", padding: "12px 24px", borderRadius: 10, fontWeight: 600, display: "inline-block", marginTop: 16 }}>Volver</Link></div></div>;
  ...
  const addWorkHoursForMember = async (memberId, hours, description, workDate) => {
    const { data } = await supabase.from("work_hours").insert({ member_id: memberId, hours, description, work_date: workDate, status: "aprobado", approved_by: member.id, approved_at: new Date().toISOString() }).select("*, members(full_name, email)").single();
    if (data) {
      setWorkHours([data, ...workHours]);
      const target = members.find(m => m.id === memberId);
      if (target) {
        const newBalance = (parseFloat(target.hours_balance) || 0) + hours;
      ...
      <div style={{ background: "#1a1a1a", borderBottom: "1px solid #333", padding: "12px 20px" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <Link href="/catalogo" style={pillD}>🛒 Catálogo</Link>
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "0 20px 40px" }}>
        ...
        {tab === "dashboard" && (<>
          <div style={{ background: s.bg, borderRadius: s.radius, padding: 20, marginBottom: 16, border: s.border }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {cycle && <button onClick={toggleCycle} style={{ ...pillD, background: cycle.status === "open" ? "var(--red)" : "var(--green-700)", color: "#fff" }}>{cycle.status === "open" ? "🔒 Cerrar" : "📦 Abrir"}</button>}
                <button onClick={createNewCycle} style={{ ...pillD, color: "var(--gold)" }}>+ Nuevo Ciclo</button>
              </div>
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12 }}>
            {[
              ...
              <div key={i} style={{ background: s.bg, borderRadius: s.radius, padding: 16, border: s.border }}>
                <div style={{ fontSize: 20, marginBottom: 6 }}>{x.i}</div>
                <div style={{ fontSize: 22, fontWeight: 700, color: x.l === "Pago pendiente" && x.v > 0 ? "var(--gold)" : "var(--gold)" }}>{x.v}</div>
        ...
        {tab === "products" && (<>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <h3 style={{ color: "var(--gold)" }}>Catálogo ({products.length})</h3>
            <button onClick={() => { setNewProd(true); setEditProd(null); }} style={{ ...pillD, background: "var(--green-700)", color: "#fff" }}>+ Agregar</button>
          </div>
          ...
          {products.map(p => {
            ...
            return (
              <div key={p.id} style={{ background: s.bg, borderRadius: 10, padding: "12px 16px", marginBottom: 6, border: s.border, display: "flex", justifyContent: "space-between", alignItems: "center", opacity: p.is_active ? 1 : 0.5 }}>
                <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => { setEditProd(p); setNewProd(false); }} style={{ ...pillD, fontSize: 11 }}>✏️</button>
                  <button onClick={async () => { await supabase.from("products").update({ is_active: !p.is_active }).eq("id", p.id); setProducts(products.map(x => x.id === p.id ? { ...x, is_active: !x.is_active } : x)); }} style={{ ...pillD, color: p.is_active ? "var(--red)" : "var(--green-700)", fontSize: 11 }}>{p.is_active ? "⏸" : "▶"}</button>
                  <button onClick={() => deleteProduct(p.id)} style={{ ...pillD, color: "var(--red)", fontSize: 11 }}>🗑</button>
                </div>
              </div>
            );
          })}
        ...
        {tab === "orders" && (<>
          ...
          {cycleOrders.length === 0 ? <p style={{ color: "#888" }}>Sin pedidos.</p> :
            cycleOrders.map(o => {
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      ...
                      {o.status !== "pendiente_pago" && o.status !== "cancelado" && (
                        <select style={selectDark} value={o.status} onChange={e => updateOrderStatus(o.id, e.target.value)}>
                          {["pagado", "preparando", "listo", "entregado", "cancelado"].map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                      )}
                    </div>
                  </div>
        ...
        {tab === "consolidated" && (<>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
            <h3 style={{ color: "var(--gold)" }}>Consolidado — Eco Río Claro</h3>
            <button onClick={exportCSV} style={{ ...pillD, background: "var(--green-700)", color: "#fff" }}>📥 Exportar CSV</button>
          </div>
          {Object.keys(consolidated).length === 0 ? <p style={{ color: "#888" }}>Sin pedidos.</p> : (
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead><tr style={{ background: "#222" }}>
                  {["Producto", "Unidad", "$ Unit.", "Hrs Unit.", "Cant.", "$ Total", "Hrs Total"].map(h => <th key={h} style={{ padding: "8px 10px", textAlign: "left", color: "var(--gold)", fontWeight: 600, borderBottom: "1px solid #444", fontSize: 11 }}>{h}</th>)}
                </tr></thead>
                <tbody>{Object.values(consolidated).map((c, i) => (
                  <tr key={i} style={{ background: i % 2 === 0 ? "#1a1a1a" : s.bg }}>
                    ...
                    <td style={{ padding: "6px 10px", fontWeight: 700, color: "var(--gold)" }}>{fmt(c.totalAmount)}</td>
        ...
        {tab === "hours" && (<>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
            <h3 style={{ color: "var(--gold)" }}>⏱ Registro de Horas de Trabajo</h3>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <span style={{ fontSize: 12, color: "#888" }}>Valor hora: {fmt(hourValue)}</span>
              <button onClick={() => setAddHoursModal({})} style={{ ...pillD, background: "var(--green-700)", color: "#fff" }}>+ Registrar horas</button>
            </div>
          </div>

          {addHoursModal && <AddHoursForm members={members} onSave={addWorkHoursForMember} onCancel={() => setAddHoursModal(null)} />}

          {workHours.length === 0 ? <p style={{ color: "#888" }}>Sin registros de horas.</p> :
            workHours.map(wh => (
              <div key={wh.id} style={{ background: s.bg, borderRadius: 10, padding: "12px 16px", marginBottom: 6, border: s.border, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                <div>
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  {wh.status === "pendiente" ? (
                    <>
                      <button onClick={() => approveWorkHours(wh)} style={{ ...pillD, background: "var(--green-700)", color: "#fff", fontSize: 11 }}>✅ Aprobar</button>
                      <button onClick={() => rejectWorkHours(wh)} style={{ ...pillD, background: "var(--red)", color: "#fff", fontSize: 11 }}>❌ Rechazar</button>
                    </>
                  ) : (
                    <span style={{ fontSize: 11, color: wh.status === "aprobado" ? "var(--green-500)" : "var(--red)" }}>{wh.status === "aprobado" ? "✅ Aprobado" : "❌ Rechazado"}</span>
                  )}
                </div>
              </div>
            ))
        ...
        {tab === "members" && (<>
          ...
          {members.map(m => (
            <div key={m.id} style={{ background: s.bg, borderRadius: 10, padding: "10px 16px", marginBottom: 6, border: s.border, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
              <div>
  ...
  return (
    <div style={{ background: "#2a2a2a", borderRadius: 10, padding: 16, marginBottom: 16, border: "1px solid #444" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        ...
        <div><label style={labD}>Categoría</label><select style={inpD} value={f.category_id} onChange={e => set("category_id", e.target.value)}>{CATEGORIES.map(c => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}</select></div>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        ...
        <button onClick={() => { if (f.name) onSave(f); }} style={{ ...pillD, background: "var(--gold)", color: "#1a1a1a" }}>💾 Guardar</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ background: "#2a2a2a", borderRadius: 10, padding: 16, marginBottom: 16, border: "1px solid #444" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        ...
        <div><label style={labD}>Horas</label><input style={inpD} type="number" step="0.25" value={hours} onChange={e => setHours(parseFloat(e.target.value) || 0)} /></div>
        <div><label style={labD}>Fecha</label><input style={inpD} type="date" value={workDate} onChange={e => setWorkDate(e.target.value)} /></div>
        <div style={{ gridColumn: "1 / -1" }}><label style={labD}>Descripción del trabajo</label><input style={inpD} value={desc} onChange={e => setDesc(e.target.value)} placeholder="Ej: Descarga de mercadería, picking de pedidos..." /></div>
      </div>
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        <button onClick={onCancel} style={pillD}>Cancelar</button>
        <button onClick={() => { if (memberId && hours > 0 && desc) onSave(memberId, hours, desc, workDate); }} style={{ ...pillD, background: "var(--gold)", color: "#1a1a1a" }}>⏱ Registrar horas</button>
      </div>
    </div>
  );
}
