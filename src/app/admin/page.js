...
export default function AdminPage() {
  ...
  const [membersSubTab, setMembersSubTab] = useState("approved");
  ...
  const toggleCycleMode = async () => {
    const newMode = cycleMode === "cycles" ? "always_open" : "cycles";
    await supabase.from("settings").update({ value: newMode }).eq("key", "cycle_mode");
    setCycleMode(newMode);
    showToast(newMode === "cycles" ? "Modo ciclos activado" : "Tienda siempre abierta");
  };
  ...
  const approveMember = async (memberId) => { await supabase.from("members").update({ approval_status: "aprobado" }).eq("id", memberId); setMembers(members.map(m => m.id === memberId ? { ...m, approval_status: "aprobado" } : m)); showToast("Cooperado aprobado"); };
  const rejectMember = async (memberId) => { await supabase.from("members").update({ approval_status: "rechazado" }).eq("id", memberId); setMembers(members.map(m => m.id === memberId ? { ...m, approval_status: "rechazado" } : m)); showToast("Cooperado rechazado"); };
  ...
  const approvalColor = { pendiente: "#e8c547", aprobado: "#40916c", rechazado: "#e63946" };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#121212", color: "#e0e0e0" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "16px auto", padding: "0 20px", display: "flex", gap: 6, overflowX: "auto" }}>
        {[["dashboard", "Dashboard"], ["stock", "Ingreso"], ["products", "Productos"], ["orders", "Pedidos"], ["consolidated", "Consolidado"], ["hours", "Horas/Turnos"], ["members", "Miembros" + (pendingApproval > 0 ? " (" + pendingApproval + ")" : "")]].map(([key, label]) => (<button key={key} onClick={() => setTab(key)} style={{ border: "none", borderRadius: 20, padding: "8px 14px", fontSize: 11, fontWeight: tab === key ? 700 : 400, background: tab === key ? "#e8c547" : key === "members" && pendingApproval > 0 ? "#e65100" : "#222", color: tab === key ? "#1a1a1a" : key === "members" && pendingApproval > 0 ? "#fff" : "#999", cursor: "pointer", whiteSpace: "nowrap" }}>{label}</button>))}
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 20px 40px" }}>
          ...
          <div style={{ background: "#1e1e1e", borderRadius: 12, padding: 20, marginBottom: 16, border: "1px solid #333" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
              <div>
                ...
                {cycleMode === "cycles" ? (
                  <div><div style={{ fontSize: 18, fontWeight: 700, marginTop: 4 }}>{cycle?.name || "Sin ciclo"}</div><div style={{ fontSize: 13, color: "#888", marginTop: 2 }}>{cycle ? cycle.start_date + " - " + cycle.end_date + " | " + cycle.status : ""}</div></div>
                ) : (
                  <div style={{ fontSize: 18, fontWeight: 700, marginTop: 4 }}>Tienda abierta permanentemente</div>
                )}
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button onClick={toggleCycleMode} style={{ ...pillD, background: "#444", color: cycleMode === "cycles" ? "#e8c547" : "#40916c" }}>{cycleMode === "cycles" ? "Cambiar a siempre abierto" : "Cambiar a ciclos"}</button>
                {cycleMode === "cycles" && (<>
                  {cycle && <button onClick={toggleCycle} style={{ ...pillD, background: cycle.status === "open" ? "#e63946" : "#2d6a4f", color: "#fff" }}>{cycle.status === "open" ? "Cerrar ciclo" : "Abrir ciclo"}</button>}
            ...
            {shifts.length === 0 ? (<p style={{ color: "#888" }}>Sin turnos.</p>) : shifts.map(shift => { const signups = shiftSignups.filter(s => s.shift_id === shift.id); const sc = shift.status === "open" ? "#2d6a4f" : shift.status === "completed" ? "#888" : "#e63946"; return (
              <div key={shift.id} style={{ background: "#1e1e1e", borderRadius: 10, padding: 16, marginBottom: 10, border: "1px solid #333" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}><span style={{ fontSize: 11, color: sc, fontWeight: 600 }}>{shift.status === "open" ? "Abierto" : shift.status === "completed" ? "Completado" : "Cerrado"}</span>{shift.status === "open" && (<><button onClick={() => closeShift(shift.id)} style={{ ...pillD, fontSize: 11, color: "#e63946" }}>Cerrar</button><button onClick={() => completeShift(shift)} style={{ ...pillD, fontSize: 11, background: "#2d6a4f", color: "#fff" }}>Completar</button></>)}</div>
                </div>
        ...
        {tab === "members" && (<div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
            <h3 style={{ color: "#e8c547" }}>Miembros ({members.length})</h3>
          </div>
          <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
            {[["pending", "Pendientes (" + pendingApproval + ")"], ["approved", "Aprobados"], ["rejected", "Rechazados"]].map(([key, label]) => (<button key={key} onClick={() => setMembersSubTab(key)} style={{ border: "none", borderRadius: 16, padding: "6px 12px", fontSize: 11, background: membersSubTab === key ? (key === "pending" ? "#e65100" : "#2d6a4f") : "#222", color: membersSubTab === key ? "#fff" : "#888", cursor: "pointer" }}>{label}</button>))}
          </div>

          {membersSubTab === "pending" && (<div>
            {members.filter(m => m.approval_status === "pendiente").length === 0 ? (<p style={{ color: "#888" }}>Sin solicitudes pendientes.</p>) :
            members.filter(m => m.approval_status === "pendiente").map(m => (
              <div key={m.id} style={{ background: "#1e1e1e", borderRadius: 10, padding: "14px 16px", marginBottom: 8, border: "1px solid #e65100", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                <div>
                  <div style={{ fontWeight: 600, fontSize: 14 }}>{m.full_name}</div>
                  <div style={{ color: "#888", fontSize: 12 }}>{m.email}</div>
                  <div style={{ color: "#888", fontSize: 11, marginTop: 2 }}>Registrado: {new Date(m.created_at).toLocaleDateString("es-CL")}</div>
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => approveMember(m.id)} style={{ ...pillD, background: "#2d6a4f", color: "#fff", fontSize: 12 }}>Aprobar</button>
                  <button onClick={() => rejectMember(m.id)} style={{ ...pillD, background: "#e63946", color: "#fff", fontSize: 12 }}>Rechazar</button>
                </div>
              </div>
            ))}
          </div>)}

          {membersSubTab === "approved" && (<div>
            {members.filter(m => m.approval_status === "aprobado" || !m.approval_status).map(m => (
              <div key={m.id} style={{ background: "#1e1e1e", borderRadius: 10, padding: "10px 16px", marginBottom: 6, border: "1px solid #333", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                ...
                <div style={{ display: "flex", gap: 12, alignItems: "center" }}><span style={{ fontSize: 12, color: "#b5651d", fontWeight: 600 }}>{fmtH(m.hours_balance)}</span><span style={{ fontSize: 11, color: "#40916c" }}>Activo</span></div>
              </div>
            ))}
          </div>)}

          {membersSubTab === "rejected" && (<div>
            {members.filter(m => m.approval_status === "rechazado").length === 0 ? (<p style={{ color: "#888" }}>Sin rechazados.</p>) :
            members.filter(m => m.approval_status === "rechazado").map(m => (
              <div key={m.id} style={{ background: "#1e1e1e", borderRadius: 10, padding: "10px 16px", marginBottom: 6, border: "1px solid #333", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                <div><span style={{ fontWeight: 600, fontSize: 14 }}>{m.full_name}</span><span style={{ color: "#888", fontSize: 12, marginLeft: 8 }}>{m.email}</span></div>
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}><span style={{ fontSize: 11, color: "#e63946" }}>Rechazado</span><button onClick={() => approveMember(m.id)} style={{ ...pillD, background: "#2d6a4f", color: "#fff", fontSize: 11 }}>Aprobar</button></div>
              </div>
            ))}
          </div>)}
        </div>)}
...
function StockEntryForm({ masterProducts, onSave, onCancel }) {
  ...
  return (<div style={{ background: "#2a2a2a", borderRadius: 10, padding: 16, marginBottom: 16, border: "1px solid #444" }}><h4 style={{ color: "#e8c547", marginBottom: 12, fontSize: 14 }}>Nuevo Ingreso</h4><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
    ...
    <div><label style={labD}>RUT</label><input style={inpD} value={supplierRut} onChange={e => setSupplierRut(e.target.value)} /></div>
    <div style={{ gridColumn: "1 / -1", display: "flex", gap: 16 }}>
      <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#ccc", cursor: "pointer" }}><input type="checkbox" checked={isOrigen} onChange={e => setIsOrigen(e.target.checked)} /><span style={{ background: "#2d6a4f", color: "#fff", fontSize: 9, padding: "2px 6px", borderRadius: 10 }}>ORIGEN</span></label>
      <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#ccc", cursor: "pointer" }}><input type="checkbox" checked={isRegenerativo} onChange={e => setIsRegenerativo(e.target.checked)} /><span style={{ background: "#6b4226", color: "#fff", fontSize: 9, padding: "2px 6px", borderRadius: 10 }}>L2M</span></label>
    </div>
  ...
  </div><div style={{ display: "flex", gap: 8, marginTop: 12 }}><button onClick={onCancel} style={pillD}>Cancelar</button><button onClick={handleSave} disabled={saving || !masterProductId || quantity <= 0 || netPrice <= 0} style={{ ...pillD, background: saving ? "#555" : "#e8c547", color: "#1a1a1a", opacity: (!masterProductId || quantity <= 0 || netPrice <= 0) ? 0.5 : 1 }}>{saving ? "..." : "Registrar"}</button></div></div>);
}
...
function AddMasterForm({ onSave, onCancel }) {
  ...
  return (<div style={{ background: "#2a2a2a", borderRadius: 10, padding: 16, marginBottom: 16, border: "1px solid #444" }}><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}><div style={{ gridColumn: "1 / -1" }}><label style={labD}>Nombre</label><input style={inpD} value={name} onChange={e => setName(e.target.value)} /></div><div><label style={labD}>Categoria</label><select style={inpD} value={categoryId} onChange={e => setCategoryId(e.target.value)}>{CATEGORIES.map(c => (<option key={c.id} value={c.id}>{c.icon} {c.name}</option>))}</select></div><div><label style={labD}>Unidad</label><input style={inpD} value={unit} onChange={e => setUnit(e.target.value)} /></div>
    <div style={{ gridColumn: "1 / -1", display: "flex", gap: 16 }}><label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#ccc", cursor: "pointer" }}><input type="checkbox" checked={isOrigen} onChange={e => setIsOrigen(e.target.checked)} /><span style={{ background: "#2d6a4f", color: "#fff", fontSize: 9, padding: "2px 6px", borderRadius: 10 }}>ORIGEN</span></label><label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#ccc", cursor: "pointer" }}><input type="checkbox" checked={isRegenerativo} onChange={e => setIsRegenerativo(e.target.checked)} /><span style={{ background: "#6b4226", color: "#fff", fontSize: 9, padding: "2px 6px", borderRadius: 10 }}>L2M</span></label></div>
...
function ProductForm({ product, onSave, onCancel }) {
  ...
  return (<div style={{ background: "#2a2a2a", borderRadius: 10, padding: 16, marginBottom: 16, border: "1px solid #444" }}><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
    ...
    <div><label style={labD}>Precio</label><input style={inpD} type="number" value={f.price} onChange={e => set("price", parseInt(e.target.value) || 0)} /></div>
    <div><label style={labD}>Horas</label><input style={inpD} type="number" step="0.01" value={f.hours_component} onChange={e => set("hours_component", parseFloat(e.target.value) || 0)} /></div>
    ...
    <div style={{ gridColumn: "1 / -1", display: "flex", gap: 16 }}><label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#ccc", cursor: "pointer" }}><input type="checkbox" checked={f.is_origen} onChange={e => set("is_origen", e.target.checked)} /><span style={{ background: "#2d6a4f", color: "#fff", fontSize: 9, padding: "2px 6px", borderRadius: 10 }}>ORIGEN</span></label><label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#ccc", cursor: "pointer" }}><input type="checkbox" checked={f.is_regenerativo} onChange={e => set("is_regenerativo", e.target.checked)} /><span style={{ background: "#6b4226", color: "#fff", fontSize: 9, padding: "2px 6px", borderRadius: 10 }}>L2M</span></label></div>
