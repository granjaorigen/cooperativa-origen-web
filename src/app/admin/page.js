...
export default function AdminPage() {
  ...
  const refreshData = async () => {
    ...
    const { data: allShifts } = await supabase.from("work_shifts").select("*").order("shift_date", { ascending: false });
    setShifts(allShifts || []);
    const { data: allSignups } = await supabase.from("shift_signups").select("*, members(full_name, email)").order("created_at", { ascending: false });
    setShiftSignups(allSignups || []);
  };
  ...
  const saveProduct = async (prod) => { if (prod.id) { await supabase.from("products").update({ name: prod.name, category_id: prod.category_id, unit: prod.unit, price: prod.price, stock: prod.stock, hours_component: prod.hours_component, is_active: prod.is_active }).eq("id", prod.id); setProducts(products.map(p => p.id === prod.id ? { ...p, ...prod } : p)); showToast("Producto actualizado"); } else { const { data } = await supabase.from("products").insert({ name: prod.name, category_id: prod.category_id, unit: prod.unit, price: prod.price, stock: prod.stock, hours_component: prod.hours_component || 0, is_active: true }).select().single(); if (data) setProducts([...products, data]); showToast("Producto agregado"); } setEditProd(null); setNewProd(false); };
  ...
  const confirmCajaPayment = async (order) => { await supabase.from("orders").update({ status: "pagado" }).eq("id", order.id); setOrders(orders.map(o => o.id === order.id ? { ...o, status: "pagado" } : o)); showToast("Pago en caja confirmado"); };
  ...
  const submitStockEntry = async (entry) => {
    try {
      ...
      if (existingProduct) {
        ...
        await supabase.from("products").update({ stock: newStock, price: entry.totalPrice }).eq("id", existingProduct.id);
        productId = existingProduct.id;
        setProducts(products.map(p => p.id === existingProduct.id ? { ...p, stock: newStock, price: entry.totalPrice } : p));
      } else {
        const { data: newP } = await supabase.from("products").insert({ name: mp.name, category_id: mp.category_id, unit: mp.unit, price: entry.totalPrice, stock: entry.quantity, hours_component: entry.hoursComponent || 0, is_active: true }).select().single();
        if (newP) { productId = newP.id; setProducts([...products, newP]); }
      }
  ...
  const addMasterProduct = async (name, categoryId, unit) => { const { data } = await supabase.from("master_products").insert({ name, category_id: categoryId, unit }).select().single(); if (data) { setMasterProducts([...masterProducts, data]); showToast("Agregado a maestra"); } setShowAddMaster(false); };
  ...
  const addWorkHoursForMember = async (memberId, hours, description, workDate) => {
    try {
      ...
      if (target) { const nb = (parseFloat(target.hours_balance) || 0) + parseFloat(hours); await supabase.from("members").update({ hours_balance: nb }).eq("id", memberId); setMembers(members.map(m => m.id === memberId ? { ...m, hours_balance: nb } : m)); }
      const { data: wh } = await supabase.from("work_hours").select("*, members(full_name, email)").order("created_at", { ascending: false });
      setWorkHours(wh || []);
      showToast("Horas registradas");
    } catch (e) { showToast("Error inesperado"); }
    setShowAddHours(false);
  };

  const createShift = async (shift) => {
    const { data, error } = await supabase.from("work_shifts").insert({ title: shift.title, description: shift.description, shift_date: shift.shiftDate, start_time: shift.startTime, end_time: shift.endTime, hours: parseFloat(shift.hours), slots: parseInt(shift.slots), status: "open", created_by: member.id }).select().single();
    if (error) { showToast("Error al crear turno"); return; }
    setShifts([data, ...shifts]);
    showToast("Turno publicado");
    setShowAddShift(false);
  };

  const closeShift = async (shiftId) => {
    await supabase.from("work_shifts").update({ status: "closed" }).eq("id", shiftId);
    setShifts(shifts.map(s => s.id === shiftId ? { ...s, status: "closed" } : s));
  ...
  const completeShift = async (shift) => {
    ...
    for (const signup of signups) {
      await supabase.from("work_hours").insert({ member_id: signup.member_id, hours: parseFloat(shift.hours), description: "Turno: " + shift.title, work_date: shift.shift_date, status: "aprobado", approved_by: member.id, approved_at: new Date().toISOString() });
      const target = members.find(m => m.id === signup.member_id);
      if (target) {
        const nb = (parseFloat(target.hours_balance) || 0) + parseFloat(shift.hours);
        await supabase.from("members").update({ hours_balance: nb }).eq("id", signup.member_id);
        setMembers(prev => prev.map(m => m.id === signup.member_id ? { ...m, hours_balance: nb } : m));
      }
      await supabase.from("shift_signups").update({ status: "completado" }).eq("id", signup.id);
    }
    await supabase.from("work_shifts").update({ status: "completed" }).eq("id", shift.id);
    setShifts(shifts.map(s => s.id === shift.id ? { ...s, status: "completed" } : s));
    setShiftSignups(shiftSignups.map(s => s.shift_id === shift.id ? { ...s, status: "completado" } : s));
    const { data: wh } = await supabase.from("work_hours").select("*, members(full_name, email)").order("created_at", { ascending: false });
    setWorkHours(wh || []);
    showToast(signups.length + " cooperados acreditados con " + fmtH(parseFloat(shift.hours)));
  };
    ...
    <div style={{ minHeight: "100vh", background: "#121212", color: "#e0e0e0" }}>
      ...
      <div style={{ background: "#1a1a1a", borderBottom: "1px solid #333", padding: "12px 20px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}><Link href="/catalogo" style={pillD}>Catalogo</Link><button onClick={handleLogout} style={pillD}>Salir</button></div>
        </div>
      </div>

      <div style={{ maxWidth: 1100, margin: "16px auto", padding: "0 20px", display: "flex", gap: 6, overflowX: "auto" }}>
        {[["dashboard", "Dashboard"], ["stock", "Ingreso"], ["products", "Productos"], ["orders", "Pedidos"], ["consolidated", "Consolidado"], ["hours", "Horas/Turnos"], ["members", "Miembros"]].map(([key, label]) => (
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 20px 40px" }}>
        ...
        {tab === "dashboard" && (<div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10 }}>
            {[{ l: "Pedidos", v: cycleOrders.length }, { l: "Pago pendiente", v: pendingPayment }, { l: "Cobrar caja", v: pendingCaja }, { l: "Cooperados", v: uniqueUsers }, { l: "Venta total", v: fmt(totalRevenue) }, { l: "Productos", v: products.filter(p => p.is_active).length }, { l: "Turnos abiertos", v: shifts.filter(s => s.status === "open").length }].map((x, i) => (
              <div key={i} style={{ background: "#1e1e1e", borderRadius: 12, padding: 14, border: "1px solid #333" }}><div style={{ fontSize: 20, fontWeight: 700, color: "#e8c547" }}>{x.v}</div><div style={{ fontSize: 10, color: "#888", marginTop: 2 }}>{x.l}</div></div>
            ))}
          </div>
        </div>)}
        ...
        {tab === "stock" && (<div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setShowAddMaster(true)} style={{ ...pillD, background: "#444", color: "#e8c547" }}>+ Maestra</button>
              <button onClick={() => setShowStockEntry(true)} style={{ ...pillD, background: "#2d6a4f", color: "#fff" }}>+ Ingreso</button>
            </div>
          </div>
          ...
          {stockEntries.length === 0 ? (<p style={{ color: "#888" }}>Sin ingresos.</p>) : stockEntries.slice(0, 20).map(se => (
            <div key={se.id} style={{ background: "#1e1e1e", borderRadius: 10, padding: "12px 16px", marginBottom: 6, border: "1px solid #333" }}>
              <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 4 }}><div><span style={{ fontWeight: 600 }}>{se.products?.name || "---"}</span><span style={{ color: "#888", fontSize: 12, marginLeft: 8 }}>x{se.quantity}</span></div><span style={{ fontSize: 12, color: "#888" }}>{new Date(se.created_at).toLocaleDateString("es-CL")}</span></div>
              <div style={{ fontSize: 12, color: "#888", marginTop: 4 }}>{se.doc_type.toUpperCase()} {se.doc_number || ""} | Neto: {fmt(se.net_price)} + IVA: {fmt(se.iva)} = {fmt(se.total_price)}/u {se.supplier_name ? "| " + se.supplier_name : ""} {se.supplier_rut ? "- " + se.supplier_rut : ""}</div>
            </div>
          ))}
          <h4 style={{ color: "#888", fontSize: 13, marginTop: 24, marginBottom: 8 }}>Maestra ({masterProducts.length})</h4>
          {masterProducts.map(mp => { const cat = CATEGORIES.find(c => c.id === mp.category_id); return (<div key={mp.id} style={{ background: "#1e1e1e", borderRadius: 8, padding: "8px 14px", marginBottom: 4, border: "1px solid #333", fontSize: 13, display: "flex", justifyContent: "space-between" }}><span>{cat?.icon} {mp.name}</span><span style={{ color: "#888" }}>{mp.unit}</span></div>); })}
        </div>)}

        {tab === "products" && (<div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}><h3 style={{ color: "#e8c547" }}>Catalogo ({products.length})</h3><button onClick={() => { setNewProd(true); setEditProd(null); }} style={{ ...pillD, background: "#2d6a4f", color: "#fff" }}>+ Agregar manual</button></div>
          ...
          {products.map(p => { const cat = CATEGORIES.find(c => c.id === p.category_id); const hc = parseFloat(p.hours_component) || 0; return (
            <div key={p.id} style={{ background: "#1e1e1e", borderRadius: 10, padding: "12px 16px", marginBottom: 6, border: "1px solid #333", display: "flex", justifyContent: "space-between", alignItems: "center", opacity: p.is_active ? 1 : 0.5 }}>
              <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600 }}>{cat?.icon} {p.name}</div><div style={{ fontSize: 12, color: "#888" }}>{fmt(p.price)} {hc > 0 ? "+ " + fmtH(hc) : ""} / {p.unit} | Stock: {p.stock}</div></div>
        ...
        {tab === "hours" && (<div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
            <h3 style={{ color: "#e8c547" }}>Horas y Turnos</h3>
            <span style={{ fontSize: 12, color: "#888" }}>Valor hora: {fmt(hourValue)}</span>
          </div>
          <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
            {[["shifts", "Turnos"], ["manual", "Registro Manual"], ["history", "Historial"]].map(([key, label]) => (
              <button key={key} onClick={() => setHoursSubTab(key)} style={{ border: "none", borderRadius: 16, padding: "6px 12px", fontSize: 11, background: hoursSubTab === key ? "#b5651d" : "#222", color: hoursSubTab === key ? "#fff" : "#888", cursor: "pointer" }}>{label}</button>
            ))}
          </div>

          {hoursSubTab === "shifts" && (<div>
            <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
              <button onClick={() => setShowAddShift(true)} style={{ ...pillD, background: "#2d6a4f", color: "#fff" }}>+ Publicar Turno</button>
            </div>
            {showAddShift && <ShiftForm onSave={createShift} onCancel={() => setShowAddShift(false)} />}
            {shifts.length === 0 ? (<p style={{ color: "#888" }}>Sin turnos.</p>) : shifts.map(shift => {
              const signups = shiftSignups.filter(s => s.shift_id === shift.id);
              const shiftStatusColor = shift.status === "open" ? "#2d6a4f" : shift.status === "completed" ? "#888" : "#e63946";
              return (
                <div key={shift.id} style={{ background: "#1e1e1e", borderRadius: 10, padding: 16, marginBottom: 10, border: "1px solid #333" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
                    <div>
                      ...
                      {shift.description && <div style={{ fontSize: 12, color: "#888", marginTop: 2 }}>{shift.description}</div>}
                      <div style={{ fontSize: 12, color: "#888", marginTop: 4 }}>
                        {shift.shift_date} {shift.start_time ? "| " + shift.start_time : ""} {shift.end_time ? "- " + shift.end_time : ""} | {fmtH(parseFloat(shift.hours))} | Cupos: {shift.slots_taken}/{shift.slots}
                      </div>
                    </div>
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      <span style={{ fontSize: 11, color: shiftStatusColor, fontWeight: 600 }}>{shift.status === "open" ? "Abierto" : shift.status === "completed" ? "Completado" : "Cerrado"}</span>
                      {shift.status === "open" && (<>
                        <button onClick={() => closeShift(shift.id)} style={{ ...pillD, fontSize: 11, color: "#e63946" }}>Cerrar</button>
                        <button onClick={() => completeShift(shift)} style={{ ...pillD, fontSize: 11, background: "#2d6a4f", color: "#fff" }}>Completar y acreditar</button>
                      </>)}
                    </div>
                  </div>
                  {signups.length > 0 && (
                    <div style={{ marginTop: 8, borderTop: "1px solid #333", paddingTop: 8 }}>
                      <div style={{ fontSize: 11, color: "#888", marginBottom: 4 }}>Inscritos:</div>
                      {signups.map(su => (
                        <div key={su.id} style={{ fontSize: 12, color: "#ccc", padding: "2px 0" }}>
                          {su.members?.full_name || "---"} <span style={{ color: "#888" }}>({su.members?.email})</span>
                          <span style={{ marginLeft: 8, fontSize: 10, color: su.status === "completado" ? "#40916c" : "#e8c547" }}>{su.status}</span>
                        </div>
                      ))}
                    </div>
                  )}
          ...
          {(hoursSubTab === "manual" || hoursSubTab === "history") && (<div>
            <h4 style={{ color: "#888", fontSize: 13, marginBottom: 8, marginTop: hoursSubTab === "history" ? 0 : 16 }}>Historial de horas registradas</h4>
            {workHours.length === 0 ? (<p style={{ color: "#888" }}>Sin registros.</p>) : workHours.slice(0, 30).map(wh => (
              <div key={wh.id} style={{ background: "#1e1e1e", borderRadius: 8, padding: "10px 14px", marginBottom: 4, border: "1px solid #333", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div><span style={{ fontSize: 13, fontWeight: 600 }}>{wh.members?.full_name || "---"}</span><span style={{ color: "#888", fontSize: 11, marginLeft: 8 }}>{wh.description} | {wh.work_date} | {fmtH(wh.hours)}</span></div>
                <span style={{ fontSize: 11, color: wh.status === "aprobado" ? "#40916c" : wh.status === "pendiente" ? "#e8c547" : "#e63946" }}>{wh.status}</span>
              </div>
            ))}
          </div>)}
        </div>)}
        ...
        {tab === "members" && (<div>
          ...
          {members.map(m => (
            <div key={m.id} style={{ background: "#1e1e1e", borderRadius: 10, padding: "10px 16px", marginBottom: 6, border: "1px solid #333", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
              <div><span style={{ fontWeight: 600, fontSize: 14 }}>{m.full_name}</span><span style={{ color: "#888", fontSize: 12, marginLeft: 8 }}>{m.email}</span>{m.is_admin && <span style={{ marginLeft: 8, background: "#e8c547", color: "#1a1a1a", padding: "1px 6px", borderRadius: 10, fontSize: 10, fontWeight: 700 }}>ADMIN</span>}</div>
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}><span style={{ fontSize: 12, color: "#b5651d", fontWeight: 600 }}>{fmtH(m.hours_balance)}</span><span style={{ fontSize: 11, color: m.is_active ? "#40916c" : "#e63946" }}>{m.is_active ? "Activo" : "Inactivo"}</span></div>
            </div>
          ))}
        </div>)}
...
function ShiftForm({ onSave, onCancel }) {
  ...
  const handleSave = async () => { if (!title.trim() || hours <= 0 || slots <= 0) return; setSaving(true); await onSave({ title: title.trim(), description: description.trim(), shiftDate, startTime, endTime, hours, slots }); setSaving(false); };
  return (
    <div style={{ background: "#2a2a2a", borderRadius: 10, padding: 16, marginBottom: 16, border: "1px solid #444" }}>
      <h4 style={{ color: "#e8c547", marginBottom: 12, fontSize: 14 }}>Publicar Nuevo Turno</h4>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <div style={{ gridColumn: "1 / -1" }}><label style={labD}>Titulo del turno</label><input style={inpD} value={title} onChange={e => setTitle(e.target.value)} placeholder="Ej: Descarga mercaderia ciclo abril" /></div>
        <div style={{ gridColumn: "1 / -1" }}><label style={labD}>Descripcion (opcional)</label><input style={inpD} value={description} onChange={e => setDescription(e.target.value)} placeholder="Ej: Descarga de frutas y verduras en bodega" /></div>
        <div><label style={labD}>Fecha</label><input style={inpD} type="date" value={shiftDate} onChange={e => setShiftDate(e.target.value)} /></div>
        <div><label style={labD}>Cupos</label><input style={inpD} type="number" min="1" value={slots} onChange={e => setSlots(parseInt(e.target.value) || 1)} /></div>
        <div><label style={labD}>Hora inicio</label><input style={inpD} type="time" value={startTime} onChange={e => setStartTime(e.target.value)} /></div>
        <div><label style={labD}>Hora fin</label><input style={inpD} type="time" value={endTime} onChange={e => setEndTime(e.target.value)} /></div>
        <div><label style={labD}>Horas a acreditar</label><input style={inpD} type="number" step="0.25" min="0.25" value={hours} onChange={e => setHours(parseFloat(e.target.value) || 0)} /></div>
      </div>
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        <button onClick={onCancel} style={pillD}>Cancelar</button>
        <button onClick={handleSave} disabled={saving || !title.trim() || hours <= 0} style={{ ...pillD, background: saving ? "#555" : "#e8c547", color: "#1a1a1a", opacity: (!title.trim() || hours <= 0) ? 0.5 : 1 }}>{saving ? "Publicando..." : "Publicar Turno"}</button>
      </div>
    </div>
  );
}
...
function StockEntryForm({ masterProducts, onSave, onCancel }) {
  ...
  const handleSave = async () => { if (!masterProductId || quantity <= 0 || netPrice <= 0) return; setSaving(true); await onSave({ masterProductId, quantity, netPrice, iva, totalPrice, docType, docNumber, supplierName, supplierRut, hoursComponent, notes }); setSaving(false); };
  return (
    <div style={{ background: "#2a2a2a", borderRadius: 10, padding: 16, marginBottom: 16, border: "1px solid #444" }}>
      <h4 style={{ color: "#e8c547", marginBottom: 12, fontSize: 14 }}>Nuevo Ingreso</h4>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        ...
        <div><label style={labD}>Horas comp. (por u.)</label><input style={inpD} type="number" step="0.01" value={hoursComponent} onChange={e => setHoursComponent(parseFloat(e.target.value) || 0)} /></div>
        <div><label style={labD}>Precio neto/u (CLP)</label><input style={inpD} type="number" value={netPrice} onChange={e => setNetPrice(parseInt(e.target.value) || 0)} /></div>
        ...
        <div style={{ gridColumn: "1 / -1", background: "#333", borderRadius: 8, padding: 10, fontSize: 13 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}><span>IVA:</span><strong>{fmt(iva)}</strong></div>
          <div style={{ display: "flex", justifyContent: "space-between", color: "#e8c547", fontSize: 15 }}><span>Total/u:</span><strong>{fmt(totalPrice)}</strong></div>
          <div style={{ display: "flex", justifyContent: "space-between", color: "#888", marginTop: 4 }}><span>Total ({quantity} u.):</span><strong>{fmt(totalPrice * quantity)}</strong></div>
        </div>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        <button onClick={onCancel} style={pillD}>Cancelar</button>
        <button onClick={handleSave} disabled={saving || !masterProductId || quantity <= 0 || netPrice <= 0} style={{ ...pillD, background: saving ? "#555" : "#e8c547", color: "#1a1a1a", opacity: (!masterProductId || quantity <= 0 || netPrice <= 0) ? 0.5 : 1 }}>{saving ? "Guardando..." : "Registrar"}</button>
      </div>
    </div>
  );
}
...
function AddMasterForm({ onSave, onCancel }) {
  const [name, setName] = useState(""); const [categoryId, setCategoryId] = useState("frutas"); const [unit, setUnit] = useState("kg");
  return (
    <div style={{ background: "#2a2a2a", borderRadius: 10, padding: 16, marginBottom: 16, border: "1px solid #444" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
        <div style={{ gridColumn: "1 / -1" }}><label style={labD}>Nombre</label><input style={inpD} value={name} onChange={e => setName(e.target.value)} /></div>
        <div><label style={labD}>Categoria</label><select style={inpD} value={categoryId} onChange={e => setCategoryId(e.target.value)}>{CATEGORIES.map(c => (<option key={c.id} value={c.id}>{c.icon} {c.name}</option>))}</select></div>
        <div><label style={labD}>Unidad</label><input style={inpD} value={unit} onChange={e => setUnit(e.target.value)} /></div>
      </div>
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}><button onClick={onCancel} style={pillD}>Cancelar</button><button onClick={() => { if (name.trim()) onSave(name.trim(), categoryId, unit); }} style={{ ...pillD, background: "#e8c547", color: "#1a1a1a" }}>Agregar</button></div>
    </div>
  );
}
...
function ProductForm({ product, onSave, onCancel }) {
  ...
  return (
    <div style={{ background: "#2a2a2a", borderRadius: 10, padding: 16, marginBottom: 16, border: "1px solid #444" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}><button onClick={onCancel} style={pillD}>Cancelar</button><button onClick={() => { if (f.name) onSave(f); }} style={{ ...pillD, background: "#e8c547", color: "#1a1a1a" }}>Guardar</button></div>
    </div>
  );
}
  ...
  return (
    <div style={{ background: "#2a2a2a", borderRadius: 10, padding: 16, marginBottom: 16, border: "1px solid #444" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        ...
        <div style={{ gridColumn: "1 / -1" }}><label style={labD}>Descripcion</label><input style={inpD} value={desc} onChange={e => setDesc(e.target.value)} placeholder="Ej: Descarga mercaderia..." /></div>
      </div>
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}><button onClick={onCancel} style={pillD}>Cancelar</button><button onClick={handleSave} disabled={saving || !memberId || hours <= 0 || !desc.trim()} style={{ ...pillD, background: saving ? "#555" : "#e8c547", color: "#1a1a1a", opacity: (!memberId || hours <= 0 || !desc.trim()) ? 0.5 : 1 }}>{saving ? "Guardando..." : "Registrar"}</button></div>
    </div>
  );
}
