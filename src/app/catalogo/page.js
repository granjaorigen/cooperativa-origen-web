...
export default function CatalogoPage() {
  ...
  const signUpForShift = async (shiftId) => {
    ...
    const alreadySignedUp = mySignups.find(s => s.shift_id === shiftId);
    if (alreadySignedUp) { showToast("Ya estas inscrito en este turno"); return; }
    const { error } = await supabase.from("shift_signups").insert({ shift_id: shiftId, member_id: member.id, status: "inscrito" });
    if (error) { console.log("Signup error:", error); showToast("Error al inscribirse"); return; }
  ...
  const cancelSignup = async (shiftId) => {
    ...
    const shift = availableShifts.find(s => s.id === shiftId);
    if (shift) {
      await supabase.from("work_shifts").update({ slots_taken: Math.max(0, shift.slots_taken - 1) }).eq("id", shiftId);
      setAvailableShifts(availableShifts.map(s => s.id === shiftId ? { ...s, slots_taken: Math.max(0, s.slots_taken - 1) } : s));
    }
  ...
  const submitOrder = async () => {
    ...
    const { data: order, error: orderError } = await supabase.from("orders").insert({
      member_id: member.id, cycle_id: cycle.id, total: grandTotal, total_hours: cartTotalHours,
      hours_paid_with_balance: hoursToUse, hours_paid_with_money: hoursToPayInMoney,
      payment_method: "pending", status: "pendiente_pago"
    }).select().single();
    ...
    const items = cartItems.map(([pid, qty]) => {
      ...
      return { order_id: order.id, product_id: pid, product_name: prod.name, product_unit: prod.unit, price: prod.price, quantity: qty, subtotal: prod.price * qty, hours_component: hc, hours_subtotal: hc * qty };
    });
    await supabase.from("order_items").insert(items);
    for (const [pid, qty] of cartItems) {
      const prod = products.find(p => p.id === pid);
      if (prod) await supabase.from("products").update({ stock: Math.max(0, prod.stock - qty) }).eq("id", pid);
    }
    if (hoursToUse > 0) {
    ...
    try {
      await fetch("/api/send-order", {
        ...
        body: JSON.stringify({ email: session.user.email, name: member?.full_name || "Cooperado", items, total: grandTotal, totalHours: cartTotalHours, hoursUsed: hoursToUse, hoursInMoney: hoursMoneyEquivalent, cycleName: cycle.name, orderId: order.id.slice(0, 8).toUpperCase(), date: new Date().toLocaleDateString("es-CL"), hourValue }),
      });
  ...
  if (loading) return (<div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontSize: 48 }}>Cargando...</span></div>);
          ...
          {loginSent ? (
            ...
            <div>
              <div style={{ marginBottom: 16 }}>
                ...
                <input style={inputStyle} value={loginName} onChange={e => setLoginName(e.target.value)} placeholder="Ej: Maria Gonzalez" />
              </div>
              ...
              <p style={{ textAlign: "center", fontSize: 12, color: "#aaa", marginTop: 12 }}>Te enviaremos un enlace seguro a tu correo. Sin contrasenas.</p>
            </div>
          )}
  ...
  // HOURS PANEL
  if (showHoursPanel) {
    const upcomingSignups = mySignups.filter(s => s.status === "inscrito");
    return (
      <div style={{ minHeight: "100vh", background: "var(--bg)", padding: 20 }}>
        <div style={{ maxWidth: 700, margin: "0 auto" }}>
          ...
          {availableShifts.length === 0 ? (
            <p style={{ color: "#888", fontSize: 14, marginBottom: 20 }}>No hay turnos disponibles en este momento.</p>
          ) : (
            <div style={{ marginBottom: 24 }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 13, marginTop: 4 }}>
                          <span style={{ color: "#b5651d", fontWeight: 600 }}>{fmtH(parseFloat(shift.hours))}</span>
                          <span style={{ color: "#888", marginLeft: 12 }}>Cupos: {shift.slots_taken}/{shift.slots}</span>
                        </div>
                      </div>
                      <div>
                        {isSignedUp ? (
                          <div style={{ textAlign: "center" }}>
                            <div style={{ fontSize: 11, color: "#2d6a4f", fontWeight: 600, marginBottom: 4 }}>Inscrito</div>
                            <button onClick={() => cancelSignup(shift.id)} style={{ ...pillBtn, background: "#f8d7da", color: "#721c24", fontSize: 11 }}>Cancelar</button>
                          </div>
                        ) : isFull ? (
                          <span style={{ fontSize: 12, color: "#999" }}>Completo</span>
                        ) : (
          ...
          {myWorkHours.length === 0 ? (
            <p style={{ color: "#888", fontSize: 14 }}>Aun no tienes horas registradas.</p>
          ) : (
            myWorkHours.map(wh => (
              <div key={wh.id} style={{ background: "#fff", borderRadius: 10, padding: "12px 16px", marginBottom: 6, border: "1px solid #eee", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>{wh.description}</div>
                  <div style={{ fontSize: 12, color: "#888" }}>{wh.work_date} | {fmtH(parseFloat(wh.hours))}</div>
                </div>
                <span style={{ fontSize: 12, fontWeight: 600, color: whStatusColors[wh.status] || "#888" }}>
                  {wh.status === "aprobado" ? "Aprobado" : wh.status === "pendiente" ? "Pendiente" : "Rechazado"}
                </span>
              </div>
            ))
          )}
              ...
              <div key={o.id} style={{ background: "#fff", borderRadius: 12, padding: 16, marginBottom: 12, border: "1px solid #eee" }}>
                ...
                {o.order_items?.map((it, i) => (
                  <div key={i} style={{ fontSize: 13, color: "#555", display: "flex", justifyContent: "space-between", padding: "2px 0" }}>
                    ...
                    <span>{fmt(it.subtotal)} {parseFloat(it.hours_subtotal) > 0 ? "+ " + fmtH(parseFloat(it.hours_subtotal)) : ""}</span>
                  </div>
                ))}
                <div style={{ borderTop: "1px solid #eee", marginTop: 8, paddingTop: 8, display: "flex", justifyContent: "space-between", fontWeight: 700 }}>
                  <span>Total</span>
                  <span style={{ color: "#2d6a4f" }}>{fmt(o.total)} {parseFloat(o.total_hours) > 0 ? "+ " + fmtH(parseFloat(o.total_hours)) : ""}</span>
                </div>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #eee", padding: "12px 20px", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <button onClick={() => setShowHoursPanel(true)} style={{ ...pillBtn, background: "#b5651d", color: "#fff", fontSize: 11 }}>
              {fmtH(hoursBalance)}
            </button>
            <button onClick={() => setShowHistory(true)} style={pillBtn}>Pedidos</button>
            <button onClick={() => setShowCart(true)} style={{ ...pillBtn, background: cartCount > 0 ? "#2d6a4f" : "#f0f0f0", color: cartCount > 0 ? "#fff" : "#333" }}>
      ...
      {showCart && (
        <div style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "flex-end" }} onClick={(e) => { if (e.target === e.currentTarget) setShowCart(false); }}>
          <div style={{ width: "100%", maxWidth: 440, background: "#fff", height: "100%", overflowY: "auto", padding: 24 }}>
            ...
            {cartItems.length === 0 ? (
              <p style={{ color: "#888", textAlign: "center", marginTop: 40 }}>Tu carrito esta vacio</p>
            ) : showPayment ? (
              <div>
                  ...
                  {hoursBalance > 0 && (
                    <div style={{ borderTop: "1px solid #ddd", paddingTop: 8, marginTop: 8 }}>
                      ...
                      {payWithHours && hoursToUse > 0 && (
                        <div style={{ fontSize: 12, color: "#2d6a4f", marginTop: 4 }}>Se descontaran {fmtH(hoursToUse)} de tu saldo</div>
                      )}
                    </div>
                  )}
                </div>
                ...
                <div style={{ background: "#2d6a4f", borderRadius: 10, padding: 16, marginBottom: 16, color: "#fff" }}>
                  ...
                  {hoursToUse > 0 && (<div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>+ {fmtH(hoursToUse)} descontadas de tu saldo</div>)}
                </div>
                <div style={{ background: "#f0f4f8", borderRadius: 10, padding: 12, marginBottom: 16, fontSize: 13, color: "#555" }}>
                  ...
                  <div style={{ marginTop: 8, fontSize: 12, color: "#888" }}>Tu pedido quedara como "Pendiente de pago" hasta que un administrador confirme el pago.</div>
                </div>
              ...
              <div>
                ...
                <div style={{ marginTop: 20, padding: "16px 0", borderTop: "2px solid #1a1a1a" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 18, fontWeight: 700 }}>
                    <span>Subtotal</span><span style={{ color: "#2d6a4f" }}>{fmt(cartTotal)}</span>
                  </div>
                  {cartTotalHours > 0 && (
